...
class UI {
  constructor(gameState) {
    ...
    this.setupDifficultyMode(); // Day 56
  }
    ...
    for (const chapter of chapters) {
      ...
      for (const levelId of chapter.levels) {
        ...
        if (isBookmarked && !isCompleted) {
          ...
          // Day 56: Hardcore completion badge
          if (levelProgress && levelProgress.hardcoreCompleted) {
            const hcBadge = document.createElement('span');
            hcBadge.className = 'hardcore-badge';
            hcBadge.textContent = '⬦';
            hcBadge.title = 'Completed in Hardcore mode';
            stars.appendChild(hcBadge);
          }
        }
  ...
  updateHintButton(tokens) {
    ...
    // Day 56: Hardcore mode hides hints entirely
    if (gs.isHardcoreMode && gs.isHardcoreMode()) {
    ...
    // Day 56: Relaxed mode shows "Free" label
    if (gs.isRelaxedMode && gs.isRelaxedMode()) {
      hintBtn.style.display = '';
      hintBtn.disabled = gs.hintsUsed >= level.hints.length;
      if (gs.hintsUsed >= level.hints.length) {
        hintBtn.textContent = '💡 No more hints';
      } else if (gs.hintsUsed === 0) {
        hintBtn.textContent = '💡 Hint (Free)';
      } else {
        hintBtn.textContent = '💡 Hint ' + (gs.hintsUsed + 1) + '/' + level.hints.length + ' (Free)';
      }
      return;
    }
  ...
  // ── Day 55: Mastery Challenges Section ──
  renderMasterySection() {
    const section = document.getElementById('mastery-section');
    const grid = document.getElementById('mastery-levels-grid');
    const countEl = document.getElementById('mastery-progress-count');
    if (!section || !grid) return;

    const progress = this.gameState.progress;
    if (typeof isCampaignComplete !== 'function' || !isCampaignComplete(progress)) {
      section.style.display = 'none';
      return;
    }

    section.style.display = '';
    const challenges = typeof getMasteryChallenges === 'function' ? getMasteryChallenges() : [];
    if (challenges.length === 0) { section.style.display = 'none'; return; }

    grid.innerHTML = '';
    let completedCount = 0;
    const masteryProgress = JSON.parse(SafeStorage.getItem('signal-circuit-mastery') || '{}');

    for (const ch of challenges) {
      const mp = masteryProgress[ch.id];
      const isComplete = mp && mp.completed;
      if (isComplete) completedCount++;

      const btn = document.createElement('div');
      btn.className = 'level-btn' + (isComplete ? ' completed mastery-completed' : '');
      btn.style.borderColor = isComplete ? '#ffd700' : '#6a3d9a';
      ...
      if (isComplete) {
        ...
        const starCount = mp.stars || 1;
        for (let i = 0; i < 3; i++) {
          const s = document.createElement('span');
          s.className = i < starCount ? 'star-filled' : 'star-empty';
          s.textContent = '\u2605';
          starsSpan.appendChild(s);
        }
        btn.appendChild(starsSpan);
      }

      const diffSpan = document.createElement('span');
      diffSpan.className = 'level-difficulty diff-hard';
      diffSpan.textContent = '\ud83d\udd34 Expert';
      btn.appendChild(diffSpan);

      btn.addEventListener('click', () => {
        this.gameState.isMasteryMode = true;
        this.gameState._currentMasteryId = ch.id;
        this.gameState.isChallengeMode = false;
        this.gameState.isSandboxMode = false;
        this.gameState._saveLevelSelectScroll();
        this.gameState.currentScreen = 'gameplay';
        this.showScreen('gameplay');
        this.gameState.audio.startAmbient();
        this.gameState.renderer.resize();
        this.gameState.renderer.resetView();
        this.gameState.loadChallengeLevel(ch);
        setTimeout(() => this.gameState.renderer.resize(), 100);
      });
    ...
    if (countEl) countEl.textContent = '(' + completedCount + '/' + challenges.length + ')';
  }

  completeMasteryChallenge(masteryId, gateCount, stars) {
    const masteryProgress = JSON.parse(SafeStorage.getItem('signal-circuit-mastery') || '{}');
    const existing = masteryProgress[masteryId];
    if (!existing || stars > (existing.stars || 0)) {
      masteryProgress[masteryId] = {
        completed: true,
        stars: stars,
    ...
    } else if (gateCount < (existing.bestGateCount || Infinity)) {
      existing.bestGateCount = gateCount;
    }
    SafeStorage.setItem('signal-circuit-mastery', JSON.stringify(masteryProgress));

    // Check if all mastery challenges complete -> unlock Logician wire color
    const challenges = typeof getMasteryChallenges === 'function' ? getMasteryChallenges() : [];
    const allComplete = challenges.every(c => masteryProgress[c.id] && masteryProgress[c.id].completed);
    if (allComplete) {
      const hasLogician = COSMETIC_WIRE_COLORS.some(c => c.id === 'logician');
      if (!hasLogician) {
        COSMETIC_WIRE_COLORS.push({
          id: 'logician', name: 'Logician', desc: 'Cyan-gold gradient — mastery reward',
          condition: null,
          palette: ['#00e5ff','#ffd700','#00bcd4','#ffcc00','#0097a7','#daa520','#26c6da','#f0c040','#00acc1','#e6b422'],
        });
      }
      if (this.gameState.achievements && this.gameState.achievements.unlock('master_logician')) {
        this.showAchievementToasts(['master_logician']);
      }
    }
  }

  // ── Day 56: Campaign Difficulty Mode UI ──
  setupDifficultyMode() {
    const btn = document.getElementById('difficulty-mode-btn');
    if (!btn) return;
    const mode = this.gameState.difficultyMode || 'standard';
    this._updateDifficultyButton(btn, mode);
    btn.addEventListener('click', () => {
      this.showDifficultySelector();
    });
  }

  _updateDifficultyButton(btn, mode) {
    const labels = { relaxed: '\ud83d\udcd8 Mode: Relaxed', hardcore: '\u26a1 Mode: Hardcore', standard: '\ud83d\udd27 Mode: Standard' };
    btn.textContent = labels[mode] || labels.standard;
    btn.className = 'settings-btn' + (mode === 'hardcore' ? ' hardcore-active' : mode === 'relaxed' ? ' relaxed-active' : '');
  }

  showDifficultySelector() {
    const gs = this.gameState;
    const currentMode = gs.difficultyMode || 'standard';
    const modal = document.getElementById('confirm-modal');
    const content = document.getElementById('confirm-modal-content');
    if (!modal || !content) return;

    const optionHtml = function(mode, emoji, label, desc, borderColor) {
      const sel = currentMode === mode;
      const bg = sel ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.03)';
      const bc = sel ? borderColor : '#333';
      return '<button class="diff-option-btn' + (sel ? ' diff-selected' : '') + '" data-mode="' + mode + '" style="text-align:left;padding:12px;background:' + bg + ';border:1px solid ' + bc + ';border-radius:8px;cursor:pointer;color:#ccc;font-family:inherit;font-size:12px;"><div style="font-size:14px;font-weight:bold;color:' + (sel ? borderColor : '#ccc') + ';">' + emoji + ' ' + label + '</div><div style="color:#888;font-size:11px;margin-top:4px;">' + desc + '</div></button>';
    };

    content.innerHTML = '<div style="text-align:center;margin-bottom:16px;"><h3 style="color:#0f0;margin-bottom:8px;">Campaign Difficulty</h3><p style="color:#888;font-size:11px;">Changes how hints, timers, and scoring work in campaign.</p></div>' +
      '<div id="diff-options" style="display:flex;flex-direction:column;gap:10px;margin-bottom:16px;">' +
      optionHtml('relaxed', '\ud83d\udcd8', 'Relaxed', 'Free unlimited hints \u2022 No timer \u2022 Encouraging messages \u2022 Skip anytime', '#2196f3') +
      optionHtml('standard', '\ud83d\udd27', 'Standard', 'Hint tokens (earn & spend) \u2022 Timer on completion \u2022 Normal star thresholds', '#0f0') +
      optionHtml('hardcore', '\u26a1', 'Hardcore', 'Zero hints \u2022 Par timer visible \u2022 Tighter stars \u2022 No skip \u2022 \u2b26 badge', '#ff4444') +
      '</div><div style="display:flex;gap:8px;justify-content:center;"><button id="diff-cancel-btn" style="background:transparent;border:1px solid #555;color:#ccc;padding:8px 20px;border-radius:6px;cursor:pointer;font-family:inherit;">Cancel</button></div>';

    modal.style.display = 'flex';

    content.querySelectorAll('.diff-option-btn').forEach(function(optBtn) {
      optBtn.addEventListener('click', function() {
        var newMode = optBtn.dataset.mode;
        gs.setDifficultyMode(newMode);
        ...
        if (settingsBtn) {
          ...
          settingsBtn.textContent = labels[newMode] || labels.standard;
          settingsBtn.className = 'settings-btn' + (newMode === 'hardcore' ? ' hardcore-active' : newMode === 'relaxed' ? ' relaxed-active' : '');
        }
        modal.style.display = 'none';
        gs.audio.playButtonClick();
      });
    ...
    var cancelBtn = content.querySelector('#diff-cancel-btn');
