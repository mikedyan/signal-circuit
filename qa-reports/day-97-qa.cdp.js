#!/usr/bin/env node
/**
 * Day 97 QA harness — Cycle 4 HARDEN Week Day 1: Full Interaction Audit.
 *
 ...
 *
 * Build under test: Day 96 (Snapshot Cards Library Tab) — last BUILD-day of Cycle 4.
 * Expected build identity: ?v=1780617600 / sw v65 / 45 level cards.
 *
 * Coverage:
 *   - Cycle 4 BUILD-week regression sweep (Days 92-96):
 ...
 *       D94 Lab Bench II Composite Constraints (L44 NAND+cap6, L45 XOR+cap5, #lab-constraint-2)
...
async function main() {
  ...
  rec('11 cache-bust refs at ?v=1780617600 (Day 96 build)', cb1 === 11, `found=${cb1}`);
  const swText = await evalExpr(ws, `fetch('/sw.js', {cache:'no-store'}).then(r=>r.text())`, { awaitPromise: true });
  rec('sw.js CACHE_NAME = signal-circuit-v65', /CACHE_NAME\s*=\s*'signal-circuit-v65'/.test(swText));
  ...
  rec('cold-start shows all 45 level cards (Day 94 added L44+L45)', coldStats && coldStats.levelCardCount === 45, `cards=${coldStats && coldStats.levelCardCount}`);
  ...
  console.log('\n=== Phase 7: Day 84+94 Lab Bench II (L41-L45) ===');
  ...
  // Day 96: Stats tabs (📸 My Cards) — verify scaffolding present, default tab is Overview
  ...
  rec('Day 96 Stats modal has 📸 My Cards tab scaffolding', statsTabs && statsTabs.tabsRootPresent && statsTabs.overviewTabPresent && statsTabs.cardsTabPresent && statsTabs.cardsPanePresent, JSON.stringify(statsTabs));
  // Cards tab badge reflects live library count (Phase 5 captured 1 via L1 solve; cold profile would show 0)
  rec('Day 96 cards tab badge format "📸 My Cards (N)"', statsTabs && /📸\s*My Cards\s*\(\d+\)/.test(statsTabs.cardsTabText), `text=${statsTabs && statsTabs.cardsTabText}`);
  ...
  rec('Day 96 clicking My Cards tab swaps panes', cardsTabSwitch && cardsTabSwitch.gridDisplay === 'none' && cardsTabSwitch.cardsPaneDisplay !== 'none', JSON.stringify(cardsTabSwitch));
