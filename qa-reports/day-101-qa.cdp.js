#!/usr/bin/env node
/**
 * Day 101 QA harness — Cycle 4 HARDEN Week, Day 5: "Regression Pass".
 *
 ...
 * Expected identity: ?v=1780617600 / sw v60 (Day 86 build, unchanged through
 * Day 87/88/89/90 HARDEN week).
 *
 * Coverage: 14-phase regression sweep across the deployed build.
 *
 *   P1  build identity on deployed site (?v=1780617600 / sw v60)
 ...
 *   P13 Day 78 staircase end-game (45 overflow + 18 nav at seed=45)
 *   P14 0 console errors across entire sweep
 *
 * Usage:
 *   NODE_PATH=/Users/openclaw/src/openclaw/node_modules \
 *     node qa-reports/day-101-qa.cdp.js
 */
...
async function main() {
  ...
  assert('P1.c — unified ?v=1780617600', buildIdentity.unified.length === 1 && buildIdentity.unified[0] === '1780617600', `unified=${JSON.stringify(buildIdentity.unified)}`);
  ...
  const swProbe = await evaluate(ws, `(async () => {
    try {
      ...
      return { swFetched: r.ok, status: r.status, len: text.length, hasV65: text.indexOf('signal-circuit-v65') >= 0, head: text.slice(0, 80) };
    } catch (e) {
  ...
  assert('P2.c — 45 level cards', coldStart.levelCardCount === 45, `got ${coldStart.levelCardCount} (expected 45 — 40 campaign + 3 Day 84 + 2 Day 94)`);
  ...
  // (live-load playthrough). For Day 101 we confirm (a) the deployed build still
  ...
  assert('P13.b — 45 overflow buttons at end-game', endgame.overflowCount === 45, `got ${endgame.overflowCount}`);
  ...
  console.log(`\n=== DAY 101 REGRESSION SWEEP (DEPLOYED) ===`);
