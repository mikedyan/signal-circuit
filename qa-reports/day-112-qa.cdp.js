#!/usr/bin/env node
/**
 * Day 112 QA harness — Cycle 5 HARDEN Week Day 1: Full Interaction Audit.
 *
 ...
 *
 * Build under test: Day 111 (Snapshot Cards Library Tab) — last BUILD-day of Cycle 5.
 * Expected build identity: ?v=1781395200 / sw v73 / 50 level cards.
 *
 * Coverage:
 *   - Cycle 5 BUILD-week regression sweep (Days 92-96):
 *       D92 Module Split Phase 1 (gates.js as ES module, window.Gate/GateTypes/IONode/roundRect)
 *       D93 Tournament Backend Worker Go-Live (remote/remote-fallback/cloud-ready modes)
 *       D94 Lab Bench II Composite Constraints (L44 NAND+cap6, L50 XOR+cap5, #lab-constraint-2)
 *       D95 Onboarding Experiment Readout UI (#onboarding-readout-card behind debug flag)
 *       D96 Snapshot Cards Library Tab (#stats-tabs, 📸 My Cards in Stats modal)
 ...
 *       stats (+ cards tab) / mastery / collection / profile / customize / settings
 *
 * Usage:
...
async function main() {
  ...
  const cb1 = (html.match(/\?v=1781395200/g) || []).length;
  rec('11 cache-bust refs at ?v=1781395200 (Day 111 build)', cb1 === 11, `found=${cb1}`);
  ...
  rec('index.html loads gates.js as ES module (Day 92)', /<script\s+type=["\']module["\']\s+src=["\']js\/gates\.js/.test(html), `gates.js module tag`);
  ...
  rec('cold-start shows all 50 level cards (Day 94 added L44+L50)', coldStats && coldStats.levelCardCount === 50, `cards=${coldStats && coldStats.levelCardCount}`);
  ...
  // ───────── PHASE 7: Day 84+94 — Lab Bench II (single + composite constraints) ─────────
  console.log('\n=== Phase 7: Day 84+94 Lab Bench II (L41-L50) ===');
    ...
    for (const id of [41,42,43,44,45]) {
      try {
        ...
        out[id] = {
          ...
          chip2Text: chip2 ? (chip2.textContent||'').trim().slice(0,80) : '',
        };
  ...
  const l44 = labCheck && labCheck['44']; const l45 = labCheck && labCheck['45'];
  ...
  // Day 94 composite levels
  rec('L44 composite: NAND-only + gateHardCap=6', l44 && l44.isLabBench && Array.isArray(l44.availableGates) && l44.availableGates[0]==='NAND' && l44.gateHardCap === 6, JSON.stringify(l44));
  rec('L44 both chips visible (composite render)', l44 && l44.chipVisible && l44.chip2Visible && /NAND/.test(l44.chipText) && /6/.test(l44.chip2Text), `c1=${l44 && l44.chipText} | c2=${l44 && l44.chip2Text}`);
  rec('L45 composite: mustIncludeGate=[XOR] + gateHardCap=5', l45 && l45.isLabBench && Array.isArray(l45.mustIncludeGate) && l45.mustIncludeGate.includes('XOR') && l45.gateHardCap === 5, JSON.stringify(l45));
  rec('L45 both chips visible (composite render)', l45 && l45.chipVisible && l45.chip2Visible && /XOR/.test(l45.chipText) && /5/.test(l45.chip2Text), `c1=${l45 && l45.chipText} | c2=${l45 && l45.chip2Text}`);
  // Composite validator: L44 with 7 NANDs should reject (cap=6)
  const compositeReject = await evalExpr(ws, `(function(){
    window.game.loadLevel(44);
    window.game.gates = [];
    window.game.wireManager.wires = [];
    for (let i=0;i<7;i++) window.game.addGate('NAND', 100+i*60, 300);
    const v = window.game._validateLabConstraints();
    return {ok: v.ok, msg: v.message || ''};
  })()`);
  rec('L44 composite validator rejects 7 NANDs (over hard cap 6)', compositeReject && compositeReject.ok === false && /hard cap of 6/.test(compositeReject.msg), JSON.stringify(compositeReject));
  ...
  // ───────── PHASE 9: Day 92 Module Split Phase 1 + Day 93 Tournament Worker regression ─────────
  console.log('\n=== Phase 9: Day 92 ES module exports + Day 93 Tournament adapter modes ===');
  ...
  rec('Day 92 window.Gate is a function', esmGlobals && esmGlobals.Gate === 'function', JSON.stringify(esmGlobals));
  rec('Day 92 window.GateTypes has 8 gate types', esmGlobals && Array.isArray(esmGlobals.GateTypesKeys) && esmGlobals.GateTypesKeys.length >= 8, JSON.stringify(esmGlobals.GateTypesKeys));
  rec('Day 92 window.IONode + roundRect re-bound', esmGlobals && esmGlobals.IONode === 'function' && esmGlobals.roundRect === 'function', JSON.stringify(esmGlobals));
  // Day 93 RemoteTournamentAdapter exists + class shape
  const remoteAdapter = await evalExpr(ws, `(function(){
    return {
      hasRemoteClass: typeof window.RemoteTournamentAdapter === 'function',
      hasLocalClass: typeof window.LocalTournamentAdapter === 'function',
      hasSelector: typeof window.selectTournamentBackend === 'function',
    };
  })()`);
  rec('Day 93 RemoteTournamentAdapter class exposed on window', remoteAdapter && remoteAdapter.hasRemoteClass, JSON.stringify(remoteAdapter));
  rec('Day 93 LocalTournamentAdapter + selectTournamentBackend present', remoteAdapter && remoteAdapter.hasLocalClass && remoteAdapter.hasSelector, JSON.stringify(remoteAdapter));
  ...
  rec('Tournament default mode label is local-leaderboard copy (Day 83/93)', tour && /local leaderboard/i.test(tour.modeLabelText), `label=${tour && tour.modeLabelText}`);
  await evalExpr(ws, `window.game.ui.showScreen('level-select')`);
  await sleep(300);
  ...
  // Day 111: Stats tabs (📸 My Cards) — verify scaffolding present, default tab is Overview
  const statsTabs = await evalExpr(ws, `(function(){
    const tabsRoot = document.querySelector('#stats-tabs');
    const overviewTab = document.querySelector('#stats-tab-overview');
    const cardsTab = document.querySelector('#stats-tab-cards');
    const cardsPane = document.querySelector('#stats-cards-pane');
    const grid = document.querySelector('#stats-grid');
    return {
      tabsRootPresent: !!tabsRoot,
      overviewTabPresent: !!overviewTab,
      cardsTabPresent: !!cardsTab,
      cardsTabText: cardsTab ? (cardsTab.textContent||'').trim() : '',
      cardsPanePresent: !!cardsPane,
      cardsPaneDisplay: cardsPane ? getComputedStyle(cardsPane).display : 'no-pane',
      gridDisplay: grid ? getComputedStyle(grid).display : 'no-grid',
    };
  })()`);
  rec('Day 111 Stats modal has 📸 My Cards tab scaffolding', statsTabs && statsTabs.tabsRootPresent && statsTabs.overviewTabPresent && statsTabs.cardsTabPresent && statsTabs.cardsPanePresent, JSON.stringify(statsTabs));
  // Cards tab badge reflects live library count (Phase 5 captured 1 via L1 solve; cold profile would show 0)
  rec('Day 111 cards tab badge format "📸 My Cards (N)"', statsTabs && /📸\s*My Cards\s*\(\d+\)/.test(statsTabs.cardsTabText), `text=${statsTabs && statsTabs.cardsTabText}`);
  // Click 📸 My Cards tab and verify pane swap
  const cardsTabSwitch = await evalExpr(ws, `(function(){
    const tab = document.querySelector('#stats-tab-cards');
    if (!tab) return {missing:true};
    tab.click();
    return {
      gridDisplay: getComputedStyle(document.querySelector('#stats-grid')).display,
      cardsPaneDisplay: getComputedStyle(document.querySelector('#stats-cards-pane')).display,
    };
  })()`);
  rec('Day 111 clicking My Cards tab swaps panes', cardsTabSwitch && cardsTabSwitch.gridDisplay === 'none' && cardsTabSwitch.cardsPaneDisplay !== 'none', JSON.stringify(cardsTabSwitch));
  ...
  // ───────── PHASE 28c: Day 95 Onboarding Readout UI (debug-gated) ─────────
  console.log('\n=== Phase 28c: Day 95 Onboarding Readout (debug-gated) ===');
  // Default: debug flag absent → developer section hidden
  const readoutDefault = await evalExpr(ws, `(function(){
  ...
  rec('Day 95 Developer section hidden by default (debug flag absent)', readoutDefault && readoutDefault.devPresent && readoutDefault.devDisplay === 'none', JSON.stringify(readoutDefault));
  await evalExpr(ws, `(document.querySelector('#settings-close')||{click:()=>{}}).click(); 'ok'`);
  await sleep(200);
  // Set debug flag → developer section + readout card visible
  const readoutDebug = await evalExpr(ws, `(function(){
    ...
    return {
      ...
      cardText: card ? (card.textContent||'').trim().slice(0,200) : '',
    };
  })()`);
  rec('Day 95 debug=1 reveals Developer section + readout card', readoutDebug && readoutDebug.devDisplay !== 'none' && readoutDebug.cardPresent && readoutDebug.cardDisplay !== 'none', JSON.stringify(readoutDebug).slice(0,260));
  rec('Day 95 readout card text mentions silent-standard variant', readoutDebug && /silent-standard/.test(readoutDebug.cardText), `text=${readoutDebug && readoutDebug.cardText.slice(0,100)}`);
  // Cleanup
